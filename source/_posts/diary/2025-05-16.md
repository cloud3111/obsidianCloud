...
- spring框架运用到了很多模式: 
	- 单例模式: 容器启动后只会创建一个 `UserService` 对象，后续从 IOC 容器获取的都是同一个实例
	- 工厂模式: Spring 的 IOC 容器本质上就是一个大型工厂
	- 策略模式: Spring 中大量使用策略模式来消除大量的 if-else; 比如说上传服务 支付服务
	- 代理模式: AOP 底层就是代理模式; jdk代理有接口, cglib代理无接口
...
- bean的是生命周期: 1实例化创建对象 2注入 3初始化(bean后置处理) 4使用 5销毁destroy
- JVM 类的生命周期包括：加载、验证、准备、解析、初始化、使用和销毁
- 为什么不推荐使用@Autowired? 
	- 因为不能想构造器那样使用final注入不可变对象, 不安全
	- 因为是隐式的, 无法看到具体使用到了那些依赖, 不利于拍错, 比如说循环依赖问题
- @Resource和@Autowired的区别? 
	- @Autowired是spring框架的, 优先按照对象类型注入, 如果有多个同一类型的报错建议使用另一个@Qualifier 注解来配合指定具体的 bean 名称实现注入
...
- 事务隔离级别: 读未提交 读已提交(解决脏读) 可重复读(解决脏读,不可重复读, MySQL默认) 串行化(解决所有,效率最低)
...
```
- 默认savelog使用require, 就是说外层register发送异常回滚, 里层savelog也会回滚
- 但是savelog使用require_new, 里外层都互不干扰, 就算register发送异常回滚, savelog也会正常记录日志
- 实际开发中最常用的是 REQUIRED 和 REQUIRES_NEW，例如业务数据和操作日志通常使用 REQUIRES_NEW 保证日志能够独立提交