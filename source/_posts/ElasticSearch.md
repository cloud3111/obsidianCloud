...
- `_doc` 是文档接口，用于根据文档 ID 对单条数据进行增删改查；`_search` 是搜索接口，用于通过 DSL 条件查询、多条件过滤、分页、排序和全文检索等操作。简单来说，`_doc` 面向单个文档，`_search` 面向文档集合。`_doc` 类似主键查询，`_search` 类似 SQL 条件查询。
| MySQL         | Elasticsearch |
| ------------- | ------------- |
| Database（数据库） | 没有完全对应物       |
| Table（表）      | Index（索引）     |
| Row（行）        | Document（文档）  |
| Column（列）     | Field（字段）     |
| 接口        | 作用     | 类比 MySQL                              |
| --------- | ------ | ------------------------------------- |
| `_doc`    | 操作单个文档 | `SELECT * FROM user WHERE id=1`       |
| `_search` | 条件搜索文档 | `SELECT * FROM user WHERE name='Tom'` |
